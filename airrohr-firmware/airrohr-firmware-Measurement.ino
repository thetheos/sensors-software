...
namespace cfg {
  ...
  char measur_influx[100] = MEASUR_INFLUX; //THETHEOS
...
#define strcpyFromJSON(key) if (json.containsKey(#key)) strcpy(key, json[#key]);
          ...
          strcpyFromJSON(measur_influx); //THETHEOS
...
#define copyToJSON_String(varname) json_string += Var2Json(#varname,String(varname));
  ...
  copyToJSON_String(measur_influx);//THETHEOS
...
void webserver_config() {
	...
	if (server.method() == HTTP_GET) {
      ...
      page_content += form_input("measur_influx", "Measurement", measur_influx, capacity_null_terminated_char_array(measur_influx)); //THETHEOS MODIFIE ICI
...
#define readPasswdParam(param) \
      ...
      readCharParam(measur_influx);//THETHEOS
...
#undef readPasswdParam
    ...
    page_content += line_from_value("Measurement", measur_influx);//THETHEOS
...
String create_influxdb_string(const String& data) {
	...
	if (json2data.success()) {
    data_4_influxdb += cfg::measur_influx; //THETHEOS Removed feinstaub
